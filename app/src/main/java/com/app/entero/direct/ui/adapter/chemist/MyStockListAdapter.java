...
public class MyStockListAdapter extends RecyclerView.Adapter<MyStockListAdapter.HolderNavigation> {
    ...
    public MyStockListAdapter(Activity activity, OnItemRecycleClickListener onItemRecycleClickListener, ArrayList<StockListModel> mOfferList) {
    ...
    public void onBindViewHolder(@NonNull HolderNavigation holderNavigation, int position) {
        ...
        holderNavigation.count_Tv.setText(mStockListModel.getStockListPrice());
        if(position==2 || position==5 || position==6)
        {
            holderNavigation.img_icon.setVisibility(View.VISIBLE);
        }
        else {
            holderNavigation.img_icon.setVisibility(View.INVISIBLE);
        }
